...
class MultiMetricLoss(object):
    ...
    def _init_multitask_weights(self):
        """初始化多任务权重和状态"""
        # [data_loss_weight, res_loss_weight, orthogonal_loss_weight]
        # [data_loss_active, res_loss_active, orthogonal_loss_active]
        loss_weights = getattr(self.args, "loss_weights", [1.0, 1.0, 1.0])
        loss_active = getattr(self.args, "loss_active", [True, True, True])

        if len(loss_weights) != 3:
            logger.warning(f"loss_weights length is {len(loss_weights)}, expected 3. Using defaults.")
            loss_weights = [1.0, 1.0, 1.0]

        if len(loss_active) != 3:
            logger.warning(f"loss_active length is {len(loss_active)}, expected 3. Using defaults.")
            loss_active = [True, True, True]

        self.data_weight = loss_weights[0]
        self.res_weight = loss_weights[1]
        self.orthogonal_weight = loss_weights[2]  # 改名从 mi_weight

        self.data_loss_active = loss_active[0]
        self.res_loss_active = loss_active[1]
        self.orthogonal_loss_active = loss_active[2]  # 改名从 mi_loss_active
        ...
        self.active_tasks = [
            ...
            self.orthogonal_loss_active,  # 改名从 mi_loss_active
        ]
        ...
        logger.info(
            f"Initial loss weights: data={self.data_weight}, res={self.res_weight}, orthogonal={self.orthogonal_weight}"
        )

        if self.use_dwa and self.num_active_tasks > 1:
            initial_weights = [self.data_weight, self.res_weight, self.orthogonal_weight]  # 改名从 mi_weight
    ...
    def compute_multitask_loss(self, im, y_true, res_loss, orthogonal_loss):
        """
        计算多任务损失
        Args:
            im: 模型输出
            y_true: 真实值
            res_loss: 残差损失
            orthogonal_loss: 正交损失 (原mi_loss)
        Returns:
            total_loss, loss_dict, metrics_dict
        """
        data_loss = self.compute_data_loss(im, y_true)

        all_losses = [data_loss, res_loss, orthogonal_loss]
        loss_names = ["data", "res", "orthogonal"]
        ...
        if self.use_dwa and len(active_losses) > 1:
            ...
            logger.debug(
                f"DWA updated weights: data={self.data_weight:.4f}, res={self.res_weight:.4f}, mi={self.mi_weight:.4f}"
            )
        else:
        ...
        loss_dict = {
            ...
            "data": data_loss,
            "res": res_loss,
            "orthogonal": orthogonal_loss,  # 改名从 mi
            ...
            "weights": {
                ...
                "orthogonal": self.orthogonal_weight if self.orthogonal_loss_active else 0.0,  # 改名
            },
            "active_status": {
                "data": self.data_loss_active,
                "res": self.res_loss_active,
                "orthogonal": self.orthogonal_loss_active,  # 改名
            },
        }
        ...
        logger.debug(
            f"  Individual losses - data: {data_loss.item():.6f}, res: {res_loss.item():.6f}, orthogonal: {orthogonal_loss.item():.6f}"
        )
        logger.debug(
            f"  Current weights - data: {self.data_weight:.4f}, res: {self.res_weight:.4f}, orthogonal: {self.orthogonal_weight:.4f}"
        )
    ...
    def __call__(self, *args, return_all_metrics=True, **kwargs):
        ...
        多任务模式: loss_fn(im, y_true, res_loss, orthogonal_loss, return_all_metrics=True)
        """
        if self.is_multitask:
            if len(args) != 4:
                raise ValueError("Multitask mode requires 4 arguments: (im, y_true, res_loss, orthogonal_loss)")

            im, y_true, res_loss, orthogonal_loss = args
            total_loss, loss_dict, metrics_dict = self.compute_multitask_loss(im, y_true, res_loss, orthogonal_loss)
