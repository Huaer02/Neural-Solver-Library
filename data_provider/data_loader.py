...
class pdebench_npy_dataset(Dataset):
    def __init__(self,
        ...
        if T_in + T_out > self.total_timesteps:
            raise ValueError(f"T_in({T_in}) + T_out({T_out}) = {T_in + T_out} > total timesteps({self.total_timesteps})")
...
class pdebench_unified_dataset(Dataset):
    def __init__(self,
        ...
        if train_ratio + val_ratio + test_ratio != 1.0:
            raise ValueError(f"train_ratio({train_ratio}) + val_ratio({val_ratio}) + test_ratio({test_ratio}) must equal 1.0")
        ...
        with h5py.File(self.file_path, "r") as h5_file:
            ...
            if len(self.original_data_shape) == 4:  # 1D: (N, T, Nx, C)
                ...
                self.dim = 3
                self.original_spatial_shape = (self.original_data_shape[2], self.original_data_shape[3], self.original_data_shape[4])
                self.spatial_shape = (
                    int(((self.original_data_shape[2] - 1) / self.downsamplex) + 1),
                    int(((self.original_data_shape[3] - 1) / self.downsampley) + 1),
        ...
        with h5py.File(self.file_path, "r") as h5_file:
            ...
            for sample_idx in self.sample_indices:
                ...
                if self.dim == 1:
                    ...
                    sample_data = np.array(tensor_data[sample_idx, :, ::self.downsamplex, ::self.downsampley, :], dtype="f")
                    sample_data = sample_data[:, :self.spatial_shape[0], :self.spatial_shape[1], :]  # (T, Nx, Ny, C)
                elif self.dim == 3:
                    sample_data = np.array(tensor_data[sample_idx, :, ::self.downsamplex, ::self.downsampley, ::self.downsamplez, :], dtype="f")
                    sample_data = sample_data[:, :self.spatial_shape[0], :self.spatial_shape[1], :self.spatial_shape[2], :]  # (T, Nx, Ny, Nz, C)
